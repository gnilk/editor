...
// Only SDL_KEYDOWN and SDL_TEXTINPUT produce a KeyPress; everything else returns empty.
//
std::optional<KeyPress> SDLKeyboardDriver::ProcessEvent(const SDL_Event &event) {
    ...
    if (event.type == SDL_EventType::SDL_TEXTINPUT) {
        ...
        // arrive as SDL_KEYDOWN with modifiers set and are handled there.
...
uint8_t SDLKeyboardDriver::TranslateModifiers(const uint16_t sdlModifiers) {
    uint8_t modifiers = 0;
    if (sdlModifiers & KMOD_LSHIFT)  modifiers |= Keyboard::kMod_LeftShift;
    if (sdlModifiers & KMOD_RSHIFT)  modifiers |= Keyboard::kMod_RightShift;
    if (sdlModifiers & KMOD_LCTRL)   modifiers |= Keyboard::kMod_LeftCtrl;
    if (sdlModifiers & KMOD_RCTRL)   modifiers |= Keyboard::kMod_RightCtrl;
    if (sdlModifiers & KMOD_LALT)    modifiers |= Keyboard::kMod_LeftAlt;
    if (sdlModifiers & KMOD_RALT)    modifiers |= Keyboard::kMod_RightAlt;
    if (sdlModifiers & KMOD_LGUI)    modifiers |= Keyboard::kMod_LeftCommand;
    if (sdlModifiers & KMOD_RGUI)    modifiers |= Keyboard::kMod_RightCommand;
    return modifiers;
}
