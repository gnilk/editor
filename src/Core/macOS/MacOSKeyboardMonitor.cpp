...
bool MacOSKeyboardMonitor::Start() {
    auto err = pthread_create(&thread, NULL, CaptureThread, this);
    if (!err) {
        return true;
    }
    if (err == EAGAIN) {
...
bool MacOSKeyboardMonitor::IsPressed(kStdControlKeys ctrlKey) {
    // Have trapped this one - return it, otherwise assume it is false...
    // Might not work for caps lock - but it is not a special key anyway...
    if (keyPressStatus.find(ctrlKey) != keyPressStatus.end()) {
        return keyPressStatus[ctrlKey];
    }
    return false;
}
...
static std::map<uint32_t, kStdControlKeys> scancodemap = {
        ...
        {0xE1, kKeyCtrl_LeftShift},
        {0xE2, kKeyCtrl_LeftAlt},
        {0xE3, kKeyCtrl_LeftCommand},   // called 'Left GUI' in origial code...
        {0xE4, kKeyCtrl_RightCtrl},
        {0xE5, kKeyCtrl_RightShift},
        {0xE6, kKeyCtrl_RightAlt},
        {0xE7, kKeyCtrl_RightCommand},
};
...
///////
//
// This is the actual keyboard monitor
// NOTE: Requires macOS to have "Monitor Input" for the terminal application
//
// Original code from: https://github.com/theevilbit/macos
//
